...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  foreach ($_POST['ID'] as $ID => $value) {
    if ($ID < 0 and $_POST['kategori_adi'][$ID] <> "") {
      ...
      $SQL = "INSERT INTO ref_odeme_kategoriler SET siralama = :siralama, ad = :kategori_adi";
    ...
    $SQL = "UPDATE ref_odeme_kategoriler SET siralama = :siralama, ad = :kategori_adi WHERE id = :id";
    $SORGU = $DB->prepare($SQL);
    $SORGU->bindParam(':siralama',    $_POST['siralama'][$ID]);
    $SORGU->bindParam(':kategori_adi', $_POST['kategori_adi'][$ID]);
    ...
    if( $_POST['kategori_adi'][$ID] == "sil") {
      $SQL = "DELETE FROM ref_odeme_kategoriler WHERE id = :id";
      $SORGU = $DB->prepare($SQL);
      $SORGU->bindParam(':id',          $_POST['ID'][$ID]);
      $SORGU->execute();
    }
...
<div class='row text-center'>
  <h1 class='alert alert-primary'>Ödeme Kategorileri</h1>
</div>
...
<form method="POST" autocomplete="off">
  <table class="table table-bordered table-hover table-striped table-sm">
    ...
    <tbody>
      ...
      $SORGU = $DB->prepare("SELECT * FROM ref_odeme_kategoriler ORDER BY siralama");
      $SORGU->execute();
